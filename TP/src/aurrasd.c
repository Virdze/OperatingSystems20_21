...
void sigchld_handler(int signum) { // Da update na disponibilidade dos filtros depois de o pChild executar
    int status;
...
int main(int argc, char const *argv[]) {
    if (argc == 1) {
        ...
        if (mkfifo(SERVER_FIFO, 0666) < 0) { // Cria pipe para os pedidos
            perror("Error creating server pipe");
            return -1;
        }

        if (mkfifo(STATUS_FIFO, 0666) < 0) { //Cria pipe para o estado
